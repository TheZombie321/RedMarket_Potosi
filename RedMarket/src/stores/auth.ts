...
export const useAuthStore = defineStore('auth', () => {
  ...
  const refreshUser = async () => {
    ...
    try {
      const data = await apiFetch<{ user: any }>('/user', { timeout: 8000, silent: true })
      user.value = data.user
      localStorage.setItem('user', JSON.stringify(data.user))
    } catch {
      // Only clear if the token hasn't changed (user didn't log in during the call)
  ...
  return { validating, user, token, login, register, logout, refreshUser, updateUser }
})
