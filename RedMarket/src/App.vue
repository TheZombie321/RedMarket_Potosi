  ...
  <div class="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div class="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
        <div class="flex items-center gap-6">
          ...
          <nav class="hidden sm:flex items-center gap-4 text-sm">
            <RouterLink to="/tienda" class="text-gray-700 hover:text-red-700 no-underline font-medium">Tienda</RouterLink>
            <input v-model="search.query" @input="router.push('/tienda')" placeholder="Buscar productos…" class="w-48 lg:w-64 px-3 py-1.5 border border-gray-300 rounded text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-300 focus:border-red-500" />
          </nav>
        </div>

        <div class="flex items-center gap-4">
          <!-- Loading state while validating session -->
          <div v-if="auth.validating" class="flex items-center gap-2 text-sm text-gray-400">
            <span class="w-7 h-7 rounded-full bg-gray-300 animate-pulse"></span>
            <span class="hidden sm:inline w-16 h-3 bg-gray-200 rounded animate-pulse"></span>
          </div>

          <!-- User dropdown -->
          <div v-else-if="auth.user" class="relative" @click.stop>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showWarning" class="fixed inset-0 z-[9998] flex items-center justify-center bg-black/40">
