...
const fetchPedidos = async () => {
  ...
  try {
    const resp = await apiFetch<any>('/pedidos', { signal: fetchAbort.signal } as any)
    todosPedidos.value = resp.data ?? resp
    await nextTick()
...
onMounted(() => {
  ...
  if (route.query.payment === 'success') {
    toast.add('Pago realizado con exito. Tu pedido esta en proceso.', 'success')
              ...
              <div class="grid grid-cols-2 gap-3 text-sm text-ink-muted">
                ...
                <p><span class="font-medium text-ink">Pago:</span>
                  <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium"
                    ...
                    {{ p.payment_method === 'stripe' ? 'Tarjeta' : 'Efectivo' }}
                ...
                <div class="mt-2">
                  <a :href="`${apiBase}/pedidos/${p.id}/ticket?token=${auth.token}`" target="_blank"
                    class="text-xs text-blue-600 hover:text-blue-800 underline">🧾 Descargar ticket</a>
                </div>
              </div>
        ...
        <div v-else class="space-y-2">
            <div v-for="p in historial" :key="p.id"
              ...
              <div class="flex items-center gap-3 text-sm text-ink-muted">
                ...
                <span class="text-xs">{{ new Date(p.created_at).toLocaleDateString('es-BO') }}</span>
                <a :href="`${apiBase}/pedidos/${p.id}/ticket?token=${auth.token}`" target="_blank"
                  class="text-xs text-blue-600 hover:text-blue-800 underline">🧾</a>
              </div>
            </div>
        </div>
      </div>
