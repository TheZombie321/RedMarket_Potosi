  ...
  <div class="max-w-3xl mx-auto bg-white rounded-lg shadow p-6">
    ...
    <div v-else>
      ...
      <div v-for="item in carrito.items" :key="item.id"
        ...
        <p class="font-bold text-orange-600 sm:w-20 text-right text-sm">Bs. {{ (item.precio_venta * item.cantidad).toFixed(2) }}</p>
      ...
      <div v-if="direccionFinal()" class="flex items-center justify-between mt-6 pt-4 border-t-2 border-gray-800">
        <div>
          <p class="text-sm text-gray-500">Productos: Bs. {{ carrito.totalPagar.toFixed(2) }}</p>
          <p class="text-sm text-gray-500">Delivery: Bs. 5.00</p>
          <h3 class="text-xl font-bold mt-1">Total: Bs. {{ (carrito.totalPagar + 5).toFixed(2) }}</h3>
        </div>
