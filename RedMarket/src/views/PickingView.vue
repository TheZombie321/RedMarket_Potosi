...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-2xl font-bold text-ink">Picking — Preparación de Pedidos</h1>
        ...
        <div v-if="qrError" class="text-rojo-mercado text-sm mb-2">{{ qrError }}</div>
            ...
            <div class="flex items-center gap-2 mb-3">
              ...
              <div class="flex items-center border border-platform-edge rounded">
                ...
                <button @click="scanCantidad = Math.min(99, scanCantidad + 1)" aria-label="Aumentar cantidad" class="px-2 py-1 text-ink-muted hover:bg-gray-100 cursor-pointer border-none bg-transparent text-sm">+</button>
          </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="mostrarModal && modalProducto" class="fixed inset-0 z-modal flex items-center justify-center bg-black/40" role="dialog" aria-modal="true" aria-label="Detalle del producto"
          @keydown.escape="mostrarModal = false">
