...
const token = computed(() => {
  try {
    return new URLSearchParams(resetUrl.value.split('?')[1] || '').get('token') || ''
  } catch {
    return ''
  }
})
const resetLink = computed(() => ({ name: 'reset-password', query: { token: token.value, email: email.value } }))
...
const copiarCodigo = () => {
  try {
    const token = new URL(resetUrl.value).searchParams.get('token') || ''
          ...
          <template v-if="isDev">
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <div class="flex items-center gap-2">
                <code class="flex-1 text-lg font-bold text-center bg-yellow-100 rounded px-3 py-1">{{ token }}</code>
