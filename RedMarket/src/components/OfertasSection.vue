...
<template>
  <section v-if="ofertas.length" class="mb-8">
    <div class="flex items-center justify-between mb-4">
      ...
      <RouterLink to="/tienda?descuento=1" class="text-sm text-red-700 hover:underline font-medium no-underline">Ver todas →</RouterLink>
    </div>

    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      <div v-for="p in ofertas" :key="p.id"
        class="bg-white rounded-xl border border-gray-100 p-3 flex flex-col items-center text-center hover:shadow-md transition-shadow">
        <RouterLink :to="`/producto/${p.id}`" class="no-underline w-full">
          ...
          <div class="mt-1">
            ...
            <span class="text-sm font-bold text-red-700 ml-1">Bs. {{ p.precio_oferta }}</span>
          </div>
          <span class="inline-block mt-1 text-[10px] bg-green-100 text-green-700 font-bold px-1.5 py-0.5 rounded-full">
            -{{ descuentoPorcentaje(p) }}%
          </span>
        </RouterLink>
        <button @click="carrito.agregarAlCarrito(p); toast.add(`${p.nombre} añadido`, 'success')"
          class="mt-2 w-full bg-red-700 text-white text-xs py-1.5 rounded-lg font-semibold cursor-pointer border-none hover:bg-red-800 transition-colors"
