...
const nominatimFetch = (url: string) => fetch(url, {
  headers: { 'User-Agent': 'RedMarketPotosi/1.0' },
})
...
const searchNominatim = async () => {
  ...
  try {
    const res = await nominatimFetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=5&countrycodes=bo`)
...
const reverseGeocode = async (latVal: number, lngVal: number) => {
  ...
  reverseTimer = setTimeout(async () => {
    ...
    try {
      const res = await nominatimFetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latVal}&lon=${lngVal}&addressdetails=1`)
